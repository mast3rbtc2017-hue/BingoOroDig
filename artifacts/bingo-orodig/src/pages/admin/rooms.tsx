import { useListRooms, useDeleteRoom } from "@workspace/api-client-react";
    ...
    <div className="min-h-screen bg-background flex flex-col text-foreground">
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button className="bg-primary text-black">Crear Sala</Button>
        </div>
              ...
              {rooms?.map((room) => (
                <TableRow key={room.id} className="border-white/10 hover:bg-white/5 transition-colors">
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" className="text-red-400 hover:text-red-300 hover:bg-red-400/10" onClick={() => handleDelete(room.id)}>Eliminar</Button>
                  </TableCell>
                </TableRow>
              ))}
              {rooms?.length === 0 && (
