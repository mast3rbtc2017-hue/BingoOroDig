        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-card/40 backdrop-blur rounded-3xl border border-white/10 p-8 mb-8 flex flex-col md:flex-row items-center gap-8 shadow-2xl">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-primary font-medium tracking-widest uppercase mb-4">{roleLabel}</p>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4">
                ...
                <div className="bg-black/40 px-4 py-2 rounded-xl border border-white/5 flex items-center gap-2">
                  ...
                  <span className="text-white/60">Total Victorias:</span>
            ...
            >
              ...
              <div className="space-y-4">
                {transactions?.slice(0, 5).map(t => (
                  <div key={t.id} className="flex justify-between items-center p-3 bg-black/40 rounded-xl border border-white/5">
                    <div>
                      ...
                      <p className="text-xs text-white/50">{format(new Date(t.createdAt), "d MMM yyyy, HH:mm", { locale: es })}</p>
                    </div>
                ...
                {(!transactions || transactions.length === 0) && (
                  <p className="text-white/40 text-center py-4">Sin transacciones aún</p>
                )}
              </div>
            </motion.div>
              ...
              <div className="space-y-4">
                {cards?.slice(0, 5).map(c => (
                  <div key={c.id} className="flex justify-between items-center p-3 bg-black/40 rounded-xl border border-white/5">
                    <div>
                      <p className="text-white font-medium">Cartón #{c.id}</p>
                      <p className="text-xs text-white/50">Partida #{c.gameId}</p>
                    </div>
                    {c.isWinner && (
                      <div className="text-xs font-bold text-black bg-accent px-2 py-1 rounded-full uppercase">¡Ganador!</div>
                    )}
                  </div>
                ))}
                {(!cards || cards.length === 0) && (
                  <p className="text-white/40 text-center py-4">Sin cartones comprados</p>
                )}
              </div>
            </motion.div>
          </div>
