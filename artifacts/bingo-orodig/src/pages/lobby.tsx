    ...
    <div className="min-h-screen bg-background flex flex-col text-foreground relative">
      ...
      <main className="flex-1 container mx-auto px-4 py-12 relative z-10">
        <div className="flex items-center justify-between mb-12">
          <div>
            <h1 className="text-4xl font-serif font-bold text-white mb-2">Lobby de Bingo</h1>
            <p className="text-white/60">Elige tu sala y empieza a jugar</p>
          </div>
        </div>
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {room.status === 'playing' ? (
                      <Badge variant="destructive" className="bg-red-500/20 text-red-400 border-red-500/30">En Curso</Badge>
                    ) : (
                      <Badge variant="secondary" className="bg-green-500/20 text-green-400 border-green-500/30">Abierta</Badge>
                    )}
                  </div>

                  <p className="text-white/60 text-sm mb-6 flex-1">
                    {room.description || "Una experiencia premium de bingo."}
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="flex flex-col bg-black/40 rounded-xl p-3 border border-white/5">
                      <span className="text-white/40 text-xs uppercase tracking-wider mb-1 flex items-center gap-1">
                        <Coins className="w-3 h-3 text-primary" /> Premio
                      </span>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/10">
                    <div className="flex items-center gap-2 text-sm text-white/60">
                      ...
                      <span>Bola cada {room.ballInterval}s</span>
                    </div>
        ...
        {rooms?.length === 0 && !isLoading && (
          <div className="text-center py-20">
            <h3 className="text-2xl text-white/60">No hay salas activas por el momento</h3>
          </div>
        )}
      </main>
    </div>
